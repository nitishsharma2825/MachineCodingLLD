...
public record Money(Currency currency, BigDecimal amount) {
    public Money {
        ...
        if (amount.scale() > currency.getDefaultFractionDigits()) {
            throw new IllegalArgumentException(("Scale of amount %s is greater than" +
                    " no of fraction digits used with currency %s").formatted(amount, currency));
        }
    }
    ...
    public Money add(Money augend) {
        if (!this.currency.equals(augend.currency())) {
            throw new IllegalArgumentException("Currency %s of augend does not match this money's currency %s".formatted(augend.currency(), this.currency));
        }
